...
from utils import DistanceDropEdge, ImprovedDropEdge
...
class LocalEncoder(nn.Module):
    ...
    def __init__(self,
        ...
        self.imroved_drop_edge = ImprovedDropEdge(1, 1, 0.1)
    ...
    def forward(self, data: TemporalData) -> torch.Tensor:
        ...
        if self.parallel:
            ...
            for t in range(self.historical_steps):
                ...
                edge_index, edge_attr, theta_list, dv_list, d = self.imroved_drop_edge(data[f'edge_index_{t}'], data[f'edge_attr_{t}'], data, t)
            ...
            out = self.aa_encoder(x=batch.x, t=None, edge_index=batch.edge_index, edge_attr=batch.edge_attr,
                                  bos_mask=data['bos_mask'], rotate_mat=data['rotate_mat'], theta=theta_list, velocity=dv_list, d=d)
            ...
            for t in range(self.historical_steps):
                ...
                out[t] = self.aa_encoder(x=data.x[:, t], t=t, edge_index=edge_index, edge_attr=edge_attr,
                                         bos_mask=data['bos_mask'][:, t], rotate_mat=data['rotate_mat'], theta=theta_list, velocity=dv_list, d=d)
...
class AAEncoder(MessagePassing):
    ...
    def __init__(self,
        ...
        self.aerfa = nn.Parameter(torch.ones(1))
    ...
    def message(self,
        ...
        Wvu = self.beta * torch.cos(theta) - self.gamma * velocity - self.aerfa * d
        Wvu = Wvu.reshape(-1, 1)
        alpha = softmax(alpha + Wvu, index, ptr, size_i)
    ...
    def _mha_block(self,
        ...
        center_embed = self.out_proj(self.propagate(edge_index=edge_index, x=x, center_embed=center_embed,
                                                    edge_attr=edge_attr, rotate_mat=rotate_mat, size=size, theta=theta, velocity=velocity, d=d))
