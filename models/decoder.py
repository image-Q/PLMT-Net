...
from typing import Tuple  # 从 typing 模块中导入了 Tuple 类型，用于表示元组类型。

import torch
import torch.nn as nn  # 导入了 PyTorch 中的神经网络模块 torch.nn，并给它起了一个别名 nn。
import torch.nn.functional as F  # 导入了 PyTorch 中的函数模块 torch.nn.functional，并给它起了一个别名 F。

from utils import init_weights  # 从名为 utils 的模块中导入了 init_weights 函数。这个函数通常用于初始化模型的权重。
