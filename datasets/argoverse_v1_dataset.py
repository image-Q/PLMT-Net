...
def process_argoverse(split: str,
    ...
    origin = torch.tensor([av_df[19]['X'], av_df[19]['Y']], dtype=torch.float)      # 获取第 20 帧（输入的最后一帧）的AV位置
    ...
    # lane_vectors_every_time = []
    for time_i in range(20):
        # get lane features for every timestamp
        df_19 = df[df['TIMESTAMP'] == timestamps[time_i]]
        ...
        # lane_vectors_every_time.append(lane_vectors)
        data['lane_vectors_%d' % time_i] = lane_vectors
        data['is_intersections_%d' % time_i] = is_intersections
        data['turn_directions_%d' % time_i] = turn_directions
        data['traffic_controls_%d' % time_i] = traffic_controls
    ...
    # 计算每个目标的速度
    velocity = torch.zeros(num_nodes, 50, dtype=torch.float)
    for t in range(50):
        for node_i in range(num_nodes):
            if t < 49:
                p1 = positions[node_i, t]
                p2 = positions[node_i, t+1]
                d = torch.norm(p2 - p1, p=2, dim=-1)
                dt = timestamps[t+1] - timestamps[t]
                v = d / dt
                velocity[node_i, t] = v
            # 如果是最后一个点,无法计算,用倒数第二个点的速度来表示
            elif t == 49:
                velocity[node_i, 49] = velocity[node_i, 48]

    data.update({
        'x': x[:, : 20],  # [N, 20, 2]
        'positions': positions,  # [N, 50, 2]，每个节点的位置（以origin为原点）
        'velocity': velocity,   # [N, 50]，每个节点的速度（以origin为原点）
        'edge_index': edge_index,  # [2, N x N - 1]，图的邻接矩阵
        'y': y,  # [N, 30, 2]
        'num_nodes': num_nodes,
